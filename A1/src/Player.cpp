 ...
 * Default constructor
 */
Player::Player() : playerName(new std::string("Unknown Player")),
 ...
 */
Player::Player(const std::string& name) : playerName(new std::string(name)),
                                          territories(new std::vector<Territory *>()),
                                          hand(new Hand()),
                                          orders(new OrdersList()),
                                          reinforcementPool(new int(0)) {
 ...
 */
Player::Player(const Player& other) : playerName(new std::string(*other.playerName)),
 ...
 */
void Player::setName(const std::string& name) const {
  *playerName = name;
}
 ...
 */
void Player::setReinforcementPool(int armies) const {
...
void Player::removeTerritory(Territory* territory) {
  if (territory) {
    auto it = std::ranges::find(*territories, territory);
...
std::vector<Territory *> Player::toDefend() const {
  ...
  // For now, return all owned territories as arbitrary defence list
    ...
    if (territory) {
      ...
      for (Territory* adjTerritory : adjTerritories) {
        if (adjTerritory && !ownsTerritory(adjTerritory)) {
          ...
          if (std::ranges::find(attackList, adjTerritory) == attackList.end()) {
