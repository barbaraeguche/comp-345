...
void testCards() {
  ...
  auto* dummyOrders = new OrdersList();

  //Store initial deck size to verify cards are returned
  int initialDeckSize = 0;

  Deck tempDeck;
  while (tempDeck.draw() != nullptr) {
    initialDeckSize++;
  }

  hand.playAll(dummyPlayer, dummyOrders, &deck);
  (void) initialDeckSize; // you didn't seem to use this variable
