...
void testPlayers() {
  ...
  Player* player2 = new Player("Alice", deck);
  Player* player3 = new Player("Bob", deck);
  ...
  // blockade orders, nullptr is the neutral player for blockade orders
  ...
  std::cout << "\n--- Test 5: Strategy Management (toDefend/toAttack) ---" << std::endl;

  // test toDefend method
  const std::vector<Territory*> defendList1 = player1->toDefend();
  const std::vector<Territory*> defendList2 = player2->toDefend();

  std::cout << "Player 1 territories to defend (" << defendList1.size() << "):" << std::endl;
  for (const Territory* terr : defendList1) {
    if (terr) {
  ...
  for (const Territory* terr : defendList2) {
    if (terr) {
      std::cout << "  - " << terr->getName() << std::endl;
    }
  }
  ...
  for (const Territory* terr : attackList1) {
    if (terr) std::cout << "  - " << terr->getName() << std::endl;
  }

  std::cout << "Player 2 territories to attack (" << attackList2.size() << "):" << std::endl;
  for (const Territory* terr : attackList2) {
    if (terr) std::cout << "  - " << terr->getName() << std::endl;
  }

  // ==================== Test 6: Copy Constructor and Assignment Operator ====================
  std::cout << "\n--- Test 6: Copy Constructor and Assignment Operator ---" << std::endl;
